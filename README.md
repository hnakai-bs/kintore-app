...
`.env` に `NUXT_PUBLIC_FIREBASE_*` を設定すると、**未ログイン時は `/login` にリダイレクト**され、ログイン後は **Firestore** の `users/{uid}/...` にユーザー関連データを保存します。変数が **空のまま**のときは **`/setup`** のみ利用可能です。**一般ユーザー向け画面からの新規登録はありません**。新規のメール／パスワードユーザーは **Firebase Console → Authentication** または **`npm run admin:create`** で追加してください。
...
| `adminUserComments/{uid}/comments/{id}` | 管理者向けコメント（`date`・`text`・`createdAt`・編集時 `updatedAt` 等。本人は読取のみ、管理者のみ書込） |
...
**ユーザー一覧**は `userDirectory` の UID を起点に、各 `users/{uid}/settings/profile` を読み取っています（`collectionGroup("settings")` は他プロジェクトの `settings` とルール衝突しやすいため使っていません）。既存ユーザは **プロフィールを一度保存し直す**か **プロフィール画面を開く**（`userDirectory` が無ければ補完）と一覧に載ります。メールアドレスは一覧には出しません（UID で識別）。**詳細**（`/admin/users/{uid}`）でコンディション／トレーニング／ボディログ／**コメント**（管理者がユーザー向けメッセージを登録）を **タブ切替**で利用できます。コメントはユーザーの **プロフィール**画面にも表示されます。
...
**補足:** 以前デプロイした管理用の Callable Functions（`adminGetUserAuth` など）がプロジェクトに残っている場合は、[Firebase Console](https://console.firebase.google.com/) の **Functions** から削除すると、未使用エンドポイントとわずかな課金の芽を減らせます。
...
| `/vision` | ビジョン |
| `/profile` | プロフィール |
| `/admin` | 管理画面・ユーザー一覧（`admin_users/{uid}` が存在するユーザーのみ） |
| `/admin/users/:uid` | 管理 — ユーザー詳細（ログ閲覧） |

## アセット

- `public/daiki-fit.png`（または差し替え時は `rebirth.png` 等）… ヘッダーロゴ
